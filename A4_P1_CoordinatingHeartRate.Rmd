...
```{r setup, include=FALSE}
library(lmerTest);library(ggplot2) ;library(caret); library(pastecs); library(lsr); library(crqa);library(magrittr); library(dplyr); library(tidyr); library(pacman); library(groupdata2)
...
param = data.frame(Dimension, Radius, Delay)
...
Profile=drpdfromts(pitch$f0, pitch$f0,datatype = 'continuous',ws=50,radius=ans$radius)
...
files = list.files(path="Tryout", pattern = ".csv")
